...
Last, I'll show how this function handles accounts where the data can't be collected, which can happen if the user ID is wrong, if it's currently suspended by Twitter, or if it's set to private. Here, I'll try to run the recip_mentioners_network function with a user ID that's wrong (notice the automatic warning messages):
```{r mistaken account, message=F}
