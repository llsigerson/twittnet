...
getreciprocators<- function(user,preset.contacts=NULL,...){
  #first, check for some abnormal users
  userinfo<- lookup_users(user)
  if(ncol(userinfo)==0){return(NA)}
  if(userinfo$protected==TRUE|userinfo$statuses_count==0){return(NA)}
  ...
  if(nrow(tweets)==0){return(as.data.frame(matrix(nrow=0,ncol=0)))}
  ...
  mentions= data.frame("contact"= as.character(mentionees$user_ID), "mentions_given"= mentionees$count, 
                       "mentions_received"= numeric(nrow(mentionees)))
  
  if(nrow(mentions)==0){return(as.data.frame(matrix(nrow=0,ncol=0)))}
